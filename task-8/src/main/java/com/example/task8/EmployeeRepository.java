...
public interface EmployeeRepository extends JpaRepository<Employee,Long> {
    ...
    @Query(value = "select new Employee(e.firstName,e.lastName,e.email,e.nationalID,e.age,e.hireDate,e.salary,e.depRole) from Employee e inner join e.projects ep " +
            "inner join ep.workingEmployees we where ep.project_ID = ?1 ")
    List<Employee> queryAllByWorkingProjectName(long projectId);
